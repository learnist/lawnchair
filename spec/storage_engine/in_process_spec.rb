...
describe "Lawnchair::StorageEngine::InProcessStore" do
  ...
  describe "#data_store" do
    it "returns the redis cache object" do
      Lawnchair::StorageEngine::InProcess.send(:class_variable_set, "@@data_store", {"mu" => "fasa"})
      in_process_store.data_store["mu"].should == "fasa"
    end
  end
  ...
  describe "#set" do
    context "when raw is true" do
      it "sets the value" do
        in_process_store.data_store["mu"].should be_nil
    ...
    context "when raw is false" do
      it "sets the value" do
        ...
        in_process_store.data_store["mu"].should == expected_value
      end
    end
  end
  ...
  describe "exists?" do
    it "returns false when the key does not exist" do
      in_process_store.data_store.keys.should_not include("mu")
      in_process_store.exists?("mu").should be_false
    end

    it "returns true when the key exists" do
      in_process_store.data_store["mu"] = "fasa"
      in_process_store.data_store.keys.should include("mu")
  ...
  describe "#expire!" do
    it "should only expire the key specified" do
      in_process_store.data_store["mu"] = "fasa"
      in_process_store.data_store["sim"] = "ba"

      in_process_store.expire!("mu")
      in_process_store.data_store["mu"].should be_nil
      in_process_store.data_store["sim"].should == "ba"
    end
  end
end
